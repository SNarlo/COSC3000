...
g_followCamOffset = 25 # changed the distance behind the racer
g_followCamLookOffset = 10
...
class RenderingSystem:
    ...
    // 2.2 Adding Fog
    vec3 addFog(in vec3 rgb, // original colour of the pixel
                in float distance) { //camera distance to the point 
        
        float b = 0.008; //the level of fog        
        float fogAmount = 1.0 - exp( -distance * b);
        vec3 fogColour = globalAmbientLight;
    ...
    vec3 computeShading(vec3 materialColour, vec3 viewSpacePosition, vec3 viewSpaceNormal, vec3 viewSpaceLightPos, vec3 lightColour)
    {
        ...
        vec3 viewSpaceDirToLight = 	normalize(viewSpaceLightPos - viewSpacePosition); // from lab 4 
        vec3 normal_ViewSpace = normalize(viewSpaceNormal);
        float incomingIntensity = max(0.0, dot(normal_ViewSpace, viewSpaceDirToLight));
        vec3 incomingLight = incomingIntensity * lightColour; // multiply incoming light by diffuse reflectance
    ...
    def setupObjModelShader(self):
        self.objModelShader = lu.buildShader(["""
                ...
                void main() 
                {
	                ...
	                fragmentColor = vec4(toSrgb(addFog(reflectedLight, v2f_viewSpacePosition.z)), material_alpha);
                }
...
def update(dt, keyStateMap, mouseDelta):
    ...
    g_viewTarget = g_racer.position # make camera follow racer

    axis = 0
    while axis <= 2:
        g_viewPosition[axis] = g_racer.position[axis] + g_followCamOffset * (g_racer.heading[axis])
        if axis == 2:
            g_viewPosition[axis] = g_racer.position[axis] + g_followCamOffset * (g_racer.heading[axis]) + g_followCamLookOffset
        axis += 1

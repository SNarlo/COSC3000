...
class Terrain:
    ...
    grassTexture = None
    pavingTexture = None
    steepRockTexture = None
    highRockTexture = None
    mapData = None
    ...
    def render(self, view, renderingSystem):
        ...
        #lu.bindTexture(texUnit, textureId, defaultTexture=None)
        lu.bindTexture(self.T_Grass, self.grassTexture)
        lu.setUniform(self.shader, "grassTexture", self.T_Grass)
        
        #2.1 Adding more textures
        lu.bindTexture(self.T_Paving, self.pavingTexture)
        lu.setUniform(self.shader, "pavingTexture", self.T_Paving)

        lu.bindTexture(self.T_Steep, self.steepRockTexture)
        lu.setUniform(self.shader, "steepRockTexture", self.T_Steep)

        lu.bindTexture(self.T_High, self.highRockTexture)
        lu.setUniform(self.shader, "highRockTexture", self.T_High)

        lu.bindTexture(self.T_Map, self.mapData)
        lu.setUniform(self.shader, "mapData", self.T_Map)
    ...
    def load(self, imageName, renderingSystem):
        ...
        vertexShader = """
            ...
            out VertexData
            {
                ...
                vec3 v2f_worldSpaceNormal; // make an output
            ...
            void main() 
            {
                ...
                v2f_worldSpaceNormal = normalIn; // declaring v2f variables (2.1)
                v2f_xyNormScale = xyNormScale; // declaring v2f variables (2.1)
                v2f_xyOffset = xyOffset; // declaring v2f variables (2.1)
...
"""
        ...
        fragmentShader = """
            ...
            in VertexData
            {
                ...
                vec3 v2f_viewSpaceNormal;
                vec3 v2f_worldSpacePosition;
                vec3 v2f_worldSpaceNormal; // normalIn (also World Space Normal)
                vec2 v2f_xyNormScale; // 2.1
                vec2 v2f_xyOffset; // 2.1
            ...
            uniform sampler2D highRockTexture; // 2.1
            uniform sampler2D steepRockTexture; // 2.1
            uniform sampler2D pavingTexture;
            uniform sampler2D mapData; // 2.1
            ...
            void main() 
            {
                ...
                vec2 wsp = v2f_worldSpacePosition.xy;
                vec3 grassColour = texture(grassTexture, wsp*terrainTextureXyScale).xyz;
                
                materialColour = grassColour;
                
                if (v2f_height > 60) { // setting all terrain over 60 to be highRockTexture
                    vec3 highRockColour = texture(highRockTexture, wsp*terrainTextureXyScale).xyz;
                    materialColour = highRockColour;
                }
                
                //Calculate steepness
                float steepAngle = dot(v2f_worldSpaceNormal, vec3(v2f_worldSpaceNormal.x, 0.0, v2f_worldSpaceNormal.z)); 
                
                if (steepAngle < 0.5) {
                    vec3 steepRockColour = texture(steepRockTexture, wsp*terrainTextureXyScale).xyz;
                    materialColour = steepRockColour;
                }
                
            
                vec2 ws_xy_coords = vec2(v2f_worldSpacePosition.x * v2f_xyNormScale.x, v2f_worldSpacePosition.y * v2f_xyNormScale.y);
                vec2 offsetScale = v2f_xyOffset * v2f_xyNormScale;
                ws_xy_coords = (ws_xy_coords - offsetScale);
                
                float blueChannel = texture(mapData, ws_xy_coords).z;
                
                if (blueChannel == 1) { // 1 is the floor
                    vec3 pavingColour = texture(pavingTexture, wsp*terrainTextureXyScale).xyz;
                    materialColour = pavingColour;
                }
	            ...
	            // 2.2 Adding Fog
	            fragmentColor = vec4(toSrgb(addFog(reflectedLight, -v2f_viewSpacePosition.z)), 1.0);
...
"""
        ...
        #ObjModelloadTexture(fileName, basePath, srgb):
        self.grassTexture = ObjModel.loadTexture("grass2.png", "data", True)
        

        # 2.1 Adding more textures

        self.pavingTexture = ObjModel.loadTexture("paving 5.png", "data", True)
        self.steepRockTexture = ObjModel.loadTexture("grassyCliff.png", "data", True)
        self.highRockTexture = ObjModel.loadTexture("rock 2.png", "data", True)
        self.mapData = ObjModel.loadTexture("track_01.png", "data", False)
